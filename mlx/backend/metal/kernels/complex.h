...
static constexpr constant bool can_convert_scalar =
    is_convertible_v<From, To> ||
    (is_same_v<To, bfloat16_t> && is_convertible_v<From, float>) ||
    (is_same_v<From, bfloat16_t> && is_convertible_v<float, To>);

template <typename From, typename To>
static constexpr constant bool can_convert_to_complex =
    !is_complex_v<From> && can_convert_scalar<From, To>;

template <typename From, typename To>
static constexpr constant bool can_convert_complex =
    !is_same_v<From, To> && can_convert_scalar<From, To>;

template <typename From, typename To>
static constexpr constant bool can_convert_from_complex =
    !is_complex_v<To> && can_convert_scalar<From, To>;

template <typename From, typename To>
using enable_complex_scalar_t =
    enable_if_t<can_convert_to_complex<From, To>, bool>;
...
struct complex_t {
  ...
  // Constructors
  constexpr complex_t(T real, T imag) : real(real), imag(imag) {};
  constexpr complex_t() : real(0), imag(0) {};
  constexpr complex_t() threadgroup : real(0), imag(0) {};

  // Conversions from scalar types
  template <
      typename U,
      typename = typename enable_if<can_convert_to_complex<U, T>>::type>
  ...
  template <
      typename U,
      typename = typename enable_if<can_convert_complex<U, T>>::type>
  ...
  template <
      typename U,
      typename = typename enable_if<can_convert_from_complex<T, U>>::type>
...
template <typename T, typename U, enable_complex_scalar_t<U, T> = true>
