...
struct ReadWriter {
  ...
  METAL_FUNC complex_T pre_out(complex_T elem) const {
    return inv ? fft_make_complex<scalar_T>(elem.x / n, -elem.y / n) : elem;
  }
  ...
  METAL_FUNC void load_padded(
      int length,
      const device complex_T* w_k) const {
  ...
  METAL_FUNC void write_padded(
      int length,
      const device complex_T* w_k) const {
    size_t batch_idx = size_t(elem.x * grid.y) * length + elem.y * length;
    int fft_idx = elem.z;
    int m = grid.z;
    complex_T inv_factor = fft_make_complex<scalar_T>(1.0f / n, -1.0f / n);
...
METAL_FUNC void ReadWriter<float2, float>::write_padded(
  ...
  for (int e = 0; e < elems_per_thread; e++) {
    ...
    if (index < length) {
      float2 output = complex_mul<float>(seq_buf[index] * inv_factor, w_k[index]);
